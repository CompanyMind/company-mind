 ...
 * `generateMetadata`, which makes it a Server Component, and this needs a
 * client boundary.
 *
 ...
 *    post with a 303 back to /<locale>/contact?sent=1|0, and the server renders
 *    the outcome through `initialStatus`. It works.
 ...
 *
 * The hidden `locale` field is what lets the no-JS path come back in the right
 * language: the route handler has no other way to know which of the three
 * /contact pages posted, and dropping someone on the Uzbek page because they
 * had JavaScript disabled would be a rude way to end the one interaction the
 * site exists for.
 *
...
export function DesignPartnerForm({
  ...
  if (status === 'sent') {
    return (
      <div
        ref={successRef}
        role="status"
        // tabIndex -1 + the focus effect above: submitting unmounts the whole
  ...
  return (
    <div className="rounded-2xl border border-line bg-paper-raised p-6 shadow-card sm:p-8">
      ...
      >
        {/* The no-JS path's only way to say which language posted. */}
