...
export async function generateMetadata({
  ...
  return {
    ...
    ...pageLocation(locale, ROUTES.about),
  }
}
...
export default async function AboutPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const { hero, position, beliefs, stage, lab, cta } = dict.about
      ...
      <section className="border-b border-line py-20 sm:py-28">
        <div className="shell">
          ...
          {/*
            ...
            │                                                                  │
            │  1. Add `name: 'Firstname Lastname'` to the role in EVERY        │
            │     locale file (content/uz.ts, ru.ts, en.ts) and to `Dictionary`│
            │     in content/types.ts — a name is not translated, but the      │
            │     three files must stay structurally identical.                │
          ...
          <div className="relative overflow-hidden rounded-lg border border-line bg-paper-raised p-8 shadow-card sm:p-14">
            ...
            <div className="flex items-center gap-2.5">
              <span
                ...
                aria-hidden="true"
              />
              <p className="mono-label">{cta.label}</p>
            </div>
            ...
            <p className="mt-7 max-w-measure leading-relaxed text-ink-soft">{cta.body}</p>

            <div className="mt-10 flex flex-wrap items-center gap-x-6 gap-y-4">
              <MagneticButton href={localePath(locale, dict.cta.href)}>
                {dict.cta.label}
              </MagneticButton>
              <p className="font-mono text-telemetry text-ink-soft">{cta.fineprint}</p>
            </div>
          </div>
        </div>
      </section>
