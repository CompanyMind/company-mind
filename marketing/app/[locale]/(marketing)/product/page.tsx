...
export async function generateMetadata({
  ...
  return {
    ...
    // The social title/description go THROUGH the helper, never in a separate
    // `openGraph` key afterwards — see the warning in lib/metadata.ts.
    ...pageLocation(locale, ROUTES.product, {
      title: t.meta.title,
      description: t.meta.description,
    }),
  }
 ...
 * ------------------------------------------------------------------------- */
function CiteMarker({ n, label }: { n: number; label: string }) {
  return (
    <a
      id={`cite-${n}`}
      href={`#source-${n}`}
 ...
 */
function CitedClause({ text, cite, label }: { text: string; cite: number; label: string }) {
...
export default async function ProductPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (!isLocale(locale)) notFound()
  const t = getDictionary(locale).product
  const { hero, ingest, organize, ask, close } = t
      ...
      <section aria-labelledby="ask-h" className="shell py-20 sm:py-28">
        ...
        <div className="mt-14 overflow-hidden rounded-lg border border-line bg-paper-raised shadow-lift">
          <div className="p-6 sm:p-10 md:p-12">
            <span className="mono-label">{ask.demo.label}</span>
            ...
            <div className="mt-12">
              <span className="mono-label">{ask.demo.answerLabel}</span>
              <p className="mt-5 max-w-4xl text-lg leading-[1.65] text-ink sm:text-xl sm:leading-[1.6] md:text-2xl md:leading-[1.55]">
                {ask.demo.answer.map((clause) => (
                  <CitedClause
                    key={clause.cite}
                    text={clause.text}
                    cite={clause.cite}
                    label={fill(ask.demo.sourceAria, { n: clause.cite })}
                  />
                ))}
              </p>
            </div>
          ...
          <div className="p-6 sm:p-10 md:p-12">
            <span className="mono-label">
              {ask.demo.sourcesLabel} ·{' '}
              {fill(ask.demo.sourcesCount, { n: ask.demo.sources.length })}
            </span>

            {/* Real footnotes: each marker jumps here, each card jumps back. */}
            <ol className="mt-5 grid gap-3 sm:grid-cols-2">
              {ask.demo.sources.map((source) => (
                <li
                  key={source.id}
                  id={`source-${source.id}`}
                  className={cn(
        ...
        <p className="mt-5 max-w-3xl text-sm leading-relaxed text-ink-soft">{ask.demo.caption}</p>

        {/* ---- Now explain the machine that produced it. ---- */}
        <div className="mt-20 grid gap-12 md:mt-24 md:grid-cols-[0.8fr_1.2fr] md:gap-16">
          <div>
            <h3 className="text-display-sm text-ink">{ask.pipelineLabel}</h3>
        ...
        <div className="mt-16 max-w-2xl rounded-md border border-line bg-paper-raised p-6 shadow-card sm:p-8">
          <h3 className="text-display-sm text-ink">{ask.empty.title}</h3>
          <p className="mt-4 text-[0.95rem] leading-relaxed text-ink-soft">{ask.empty.body}</p>
        </div>
      ...
      >
        <div className="shell">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <MagneticButton href={localePath(locale, close.primary.href)} variant="ink">
              {close.primary.label}
            </MagneticButton>
            <MagneticButton href={localePath(locale, close.secondary.href)} variant="quiet">
