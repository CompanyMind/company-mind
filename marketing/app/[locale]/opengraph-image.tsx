 ...
 *  4. IT LIVES UNDER `[locale]`, AND IT HAS TO. A metadata image attaches to
 *     the segment tree it sits in, and since the root layout moved under
 *     `[locale]` a file at `app/opengraph-image.tsx` stopped being picked up —
 *     the generated HTML simply had no `og:image` at all, silently. Here it
 *     attaches, and the card comes out in the language of the page that was
 *     shared: three images at build time, and a Russian link previews in
 *     Russian. Verify after any routing change by grepping the built HTML for
 *     `og:image` rather than trusting that it is still wired up.
 * ------------------------------------------------------------------------- */

/**
 * `alt` must be a static export, so it cannot vary per locale. It describes the
 * card for someone who cannot see it, and the default locale is the honest
 * choice for the one string that has to be picked once.
 */
export const alt = `${brand.name} — ${getDictionary(defaultLocale).site.tagline}`
export const size = { width: 1200, height: 630 }
export const contentType = 'image/png'

/**
 * Required even though the parent layout already has one: a metadata image is
 * its own route, and without this Next cannot enumerate `[locale]` for it. The
 * symptom is `ƒ /-/opengraph-image` in the build output — a placeholder param,
 * generated on demand — instead of three prerendered cards.
 */
